...
1. Convert the necessary dimensions to measures  (the variables that are categorical in nature). When you use a separate sheet for this question, add a note in that sheet on which columns were changed.
	Columns changed to discreet: Bedrooms, bathrooms, floors, condition, grade, view, yr_built, yr_renovated
	Columns changed to Boolean (calc. field created): Waterfront, Renovated
...
7. Create calculated field `year_built_bins` for the column `year_built` by creating buckets as follows, for houses built between 1900 and 2000 - category A, for houses built between 2000 and 2010 - category B, and for houses built between 2011 and 2015 - category C. Use `IF-ELSE` statement to create the bins/buckets. Compare the prices of houses for the three categories.
...
9. Deep dive in category A, category B and category C using filters. Identify different characteristics/trends for each of the three categories.
	- Grade 13 and 12 properties only exist in Categories A and B.
	- Category C only contains properties with condition 3
	- Waterfront properties are only contained in Category A
	- Renovated properties are only contained in Category A
	- Smallest properties (Living15_SqFt) are in Category A (1,922.5 avg)
	-  
